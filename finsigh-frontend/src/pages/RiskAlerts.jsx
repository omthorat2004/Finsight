...
const RiskAlerts = () => {
  ...
  const markAsResolved = (alertId) => {
    // For college project, just update locally
    setAlerts(prev => 
      prev.map(alert => 
        alert.id === alertId 
    ...
    if (resolvedAlert) {
      setStats(prev => ({
        ...
        [resolvedAlert.severity.toLowerCase()]: Math.max(0, prev[resolvedAlert.severity.toLowerCase()] - 1)
      }));
    }
  ...
  const filteredAlerts = alerts.filter(alert => {
    ...
    if (filter !== 'all') {
      ...
      else if (filter === 'compliance') matchesFilter = alert.type === 'Compliance';
  ...
  const getTypeIcon = (type) => {
    switch(type) {
      case 'Fraud': return <FlagIcon className="h-5 w-5" />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Monitor and manage security risks in real-time</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">No risk alerts match your current filters</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 text-sm mt-1">"{alert.feedback}"</p>
                        {alert.category && (
                          <p className="text-xs text-gray-400 mt-1">Category: {alert.category}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    {alert.status !== 'resolved' && (
                      <button 
                        ...
                        className="px-3 py-1 text-green-600 text-sm hover:bg-green-50 rounded-lg"
                      >
                        Mark Resolved
                      </button>
                    )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-lg ${
                  ...
                  selectedAlert.severity === 'High' ? 'bg-orange-100' : 'bg-yellow-100'
                }`}>
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(selectedAlert.status)}`}>
              ...
              <div>
                ...
                <p className="mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">"{selectedAlert.feedback}"</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="mt-1 font-medium text-gray-900">{selectedAlert.type}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Category</label>
                  <p className="mt-1 font-medium text-gray-900">{selectedAlert.category}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Time</label>
                  <p className="mt-1 font-medium text-gray-900">{selectedAlert.time}</p>
                </div>
              </div>
